...
def resize_cover_and_center_crop_bgr(image_bgr, target_h=480, target_w=832):
    """
    等比缩放(覆盖目标尺寸) + 居中裁剪到 (target_h, target_w)。
    输入/输出：BGR, uint8
    """
    h, w = image_bgr.shape[:2]
    s = max(target_w / w, target_h / h)  # 覆盖
    new_w, new_h = int(round(w * s)), int(round(h * s))
    # 双线性/AREA 都可；放大建议 INTER_LINEAR
    image_resized = cv2.resize(image_bgr, (new_w, new_h), interpolation=cv2.INTER_LINEAR)

    # 居中裁剪
    x0 = (new_w - target_w) // 2
    y0 = (new_h - target_h) // 2
    image_cropped = image_resized[y0:y0+target_h, x0:x0+target_w]
    return image_cropped.copy()
...
@torch.no_grad()
def _count_matpix_for_pair_gim_dkm(model, device, img1_path, img2_path, tau=0.5, max_size=1024):
    """
    仅用于 gim_dkm:对一对帧做匹配,返回 conf>=tau 的匹配像素数量 Mat.Pix.
    """
    im1 = read_image(img1_path)          # RGB, uint8
    im2 = read_image(img2_path)          # RGB, uint8

    # 等比缩放覆盖 + 居中裁剪到 (480, 832)
    im1 = resize_cover_and_center_crop_bgr(im1, target_h=480, target_w=832)
    im2 = resize_cover_and_center_crop_bgr(im2, target_h=480, target_w=832)

    # 转成 GIM 期望的 [0,1] RGB 张量（不再额外 resize）
    im1_rgb = cv2.cvtColor(im1, cv2.COLOR_BGR2RGB)
    im2_rgb = cv2.cvtColor(im2, cv2.COLOR_BGR2RGB)
    im1_rgb = np.ascontiguousarray(im1_rgb)
    im2_rgb = np.ascontiguousarray(im2_rgb)

    t1 = torch.from_numpy(im1_rgb).float().permute(2,0,1)[None] / 255.0
    t2 = torch.from_numpy(im2_rgb).float().permute(2,0,1)[None] / 255.0
    t1 = t1.to(device)
    t2 = t2.to(device)

    width, height = 672, 896
    orig_width0, orig_height0, pad_left0, pad_right0, pad_top0, pad_bottom0 = get_padding_size(t1, width, height)
    orig_width1, orig_height1, pad_left1, pad_right1, pad_top1, pad_bottom1 = get_padding_size(t2, width, height)
    t1_ = torch.nn.functional.pad(t1, (pad_left0, pad_right0, pad_top0, pad_bottom0))
    t2_ = torch.nn.functional.pad(t2, (pad_left1, pad_right1, pad_top1, pad_bottom1))

    dense_matches, dense_certainty = model.match(t1_, t2_)
    ...
    mask = (kpts0[:,0] > 0) & (kpts0[:,1] > 0) & (kpts1[:,0] > 0) & (kpts1[:,1] > 0)
    mask = mask & (kpts0[:,0] <= (orig_width0 - 1)) & (kpts1[:,0] <= (orig_width1 - 1)) \
                 & (kpts0[:,1] <= (orig_height0 - 1)) & (kpts1[:,1] <= (orig_height1 - 1))
    mconf = mconf[mask]

    # 返回 Mat.Pix.（>= tau 的数量）
    if torch.is_tensor(mconf):
        mconf = mconf.detach().float().cpu().numpy()
    return int((mconf >= tau).sum())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generated_root', type=str, default=None, help='生成视频帧根目录, 包含多个 *_frames 子文件夹,如 generated_videos/')
    parser.add_argument('--real_root', type=str, default=None, help='真实视频帧根目录,结构与 generated_root 对应,如 real_videos/')
    parser.add_argument('--frame_glob', type=str, default='frame_*.jpg', help='逐帧文件名通配符,如 frame_*.jpg')
    parser.add_argument('--tau', type=float, default=0.5, help='置信度阈值')
    parser.add_argument('--max_size', type=int, default=None, help='统一最长边,None 表示不缩放')
    parser.add_argument('--csv_out', type=str, default='matpix_results.csv', help='批量评测输出 CSV 路径')
    ...
    if args.generated_root is not None and args.real_root is not None:
        ...
        for gen_dir, real_dir in pairs:
            name = os.path.basename(gen_dir)

            gen_frames = list_frames(gen_dir)
            real_frames = list_frames(real_dir)
            assert len(gen_frames) == len(real_frames) and len(gen_frames) > 0, \
                f"帧数不一致或为空：{gen_dir}({len(gen_frames)}) vs {real_dir}({len(real_frames)})；" \
                f"确认命名与 {args.frame_glob} 是否匹配"

            counts = []
            for p_gen, p_real in zip(gen_frames, real_frames):
                c = _count_matpix_for_pair_gim_dkm(
                    model, device, p_gen, p_real, tau=args.tau, max_size=args.max_size
                )
                counts.append(c)

            total = int(np.sum(counts))
            mean_ = float(np.mean(counts))
            per_video_means.append(mean_)
            rows.append({
                "video_pair": name,
                "num_pairs": len(counts),
                "tau": args.tau,
                "matpix_total": total,
                "matpix_mean": round(mean_, 3),
                "matpix_total_K": round(total / 1000.0, 3),
                "matpix_mean_K": round(mean_ / 1000.0, 3),
            })

        overall_mean = float(np.mean(per_video_means)) if per_video_means else 0.0
        rows.append({
            "video_pair": "AVERAGE_OVER_ALL",
            "num_pairs": int(np.sum([r["num_pairs"] for r in rows])) if rows else 0,
            "tau": args.tau,
            "matpix_total": "",
            "matpix_mean": round(overall_mean, 3),
            "matpix_total_K": "",
            "matpix_mean_K": round(overall_mean / 1000.0, 3),
        })

        os.makedirs(os.path.dirname(args.csv_out) or ".", exist_ok=True)
        with open(args.csv_out, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "video_pair","num_pairs","tau","matpix_total",
                "matpix_mean","matpix_total_K","matpix_mean_K"
            ])
            writer.writeheader()
            for r in rows:
                writer.writerow(r)

        print(f"[OK] 已写出 CSV: {args.csv_out}")
        exit(0)
